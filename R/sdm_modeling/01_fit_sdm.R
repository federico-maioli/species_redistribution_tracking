...
data <- data %>% 
  mutate(
    ...
    quarter = case_when(
      ...
      region_short %in% c("BC", "BS") & quarter %in% c("Q3", "Q4") ~ "Q3_Q4",
      
      # CBS: Q1 and Q3_Q4
      region_short == "CBS" & quarter %in% c("Q3", "Q4") ~ "Q3_Q4",
      
      # COW: Q2 and Q3_Q4
      region_short == "COW" & quarter %in% c("Q3", "Q4") ~ "Q3_Q4",
...
process_species_region <- function(i) {
  tryCatch({
    ...
    multi_season_regions <- c("BAL", "NS", "NEUS", "CBS", "GMX") # Identify whether the current region should use quarter
    ...
    if (has_multiple_surveys && use_quarter) {
    ...
    saveRDS(fit, file = here("sdm_modeling/fitted", paste0(this_region, "_", this_species, "_sdm.rds")))
...
process_species_region(1) # for testing a single run
