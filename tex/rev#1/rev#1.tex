...
We agree the regional differences in warming needed to be addressed and they are now reported explicitly. Rather than grouping regions by whether their warming trend was significant, which dichotomises a continuous quantity and depends on statistical power, we examined whether responses scale with regional warming rate directly. Displacement showed no detectable scaling (latitude r = −0.38, longitude r = −0.10, depth r = 0.50, all intervals spanning zero), whereas occupied temperature closely followed local warming (r = 0.92), as expected. The absence of scaling indicates our conclusions are not an artefact of including regions with little warming.
\end{reply}
