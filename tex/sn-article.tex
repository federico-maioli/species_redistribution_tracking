...
At the regional scale, estimated spatial shift slopes typically overlapped zero, indicating limited evidence for consistent directional changes in mean latitude, longitude, or depth across species within most regions (Fig.\ref{fig:posterior_slopes}b). Exceptions were observed for depth, with moderately stronger deepening trends along the U.S. West Coast (USWC;\DepthUSWCMedian{} m decade$^{-1}$ \DepthUSWCMCI{}) and the Northeast U.S.–Scotian Shelf (NEUS–SS;\DepthNEUSMedian{} m decade$^{-1}$ \DepthNEUSCI{}).
