...
We initially modeled the joint distribution of spatial and thermal responses using a multivariate normal (MVN) likelihood, motivated by the expectation that these metrics are correlated (e.g., depth shifts co-occurring with changes in geographic range centroids). However, posterior predictive checks revealed heavier-tailed behavior than could be adequately captured by the MVN distribution. To improve robustness, we therefore modeled each response using a univariate Student-t likelihood, which accommodates heavier tails while retaining well-defined location and scale parameters. Correlations among responses were preserved through a multivariate normal covariance structure on the group-level effects, allowing shared variation across responses to be modeled explicitly. This specification improved predictive performance, as assessed using leave-one-out cross-validation (LOO), with higher expected log predictive density (ELPD) relative to the MVN model (Supplementary Table~\ref{tab:elpd}; \citealp{vehtari_practical_2017}).
...
\begin{figure}[H]
    ...
    \caption{Convergence diagnostics for the hierarchical trend model. Panels show $\hat{R}$, bulk effective sample size (ESS), and tail ESS for all parameters. Yellow regions indicate values outside recommended thresholds. All parameters have $\hat{R} < 1.01$ and tail ESS $> 400$. One parameter falls slightly below the recommended bulk ESS threshold, but visual inspection of the chains indicated satisfactory mixing (not shown), confirming reliable posterior sampling.}
    \label{fig:rhat_supp}
\end{figure}
